...
namespace VHITEK
{
    namespace EEPROM
    {
        ...
        {
            user_setting read_check_sum;
            byte check_dung=0;

            static uint16_t o_luu;
            o_luu = VHITEK::Menu::tinh_o_luu_the(user_check.so_tu);
            
            user_check.check_sum=sumCalc_eeprom_1(user_check);
            accessI2C1Bus([&]{
                    myMem.put(o_luu, user_check); //luu vao eeprom             
            }, 100);    

            // Serial.print("SUM the: "); Serial.println(user_check.check_sum);
        ...
        user_setting read_eeprom_1(uint16_t so_tu) //doc eeprom 1
        {
            uint16_t Sum_cal;
            user_setting user_read_eeprom;
            uint16_t o_luu = VHITEK::Menu::tinh_o_luu_the(so_tu);

            accessI2C1Bus([&]{
                    myMem.get(o_luu, user_read_eeprom); //Doc Check SUM trong eeprom              
            }, 100);

            Sum_cal = sumCalc_eeprom_1(user_read_eeprom); //tinh CHECK SUM vua doc
