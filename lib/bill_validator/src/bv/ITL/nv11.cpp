...
void NV11::begin()
{
    ...
    _on(SSP_POLL_CREDIT, [&](SSP_POLL_EVENT_DATA data)
        {
            Serial.printf("SSP_POLL_STACKED OnEVent %d %d \n",data.data.size());
            if (data.data.size() < 1)
                return;
    ...
    _addPollEvent(SSP_POLL_DISPENSING,
                  SSP_POLL_INFO,
                  "SSP_POLL_DISPENSING",
                  -1, F("The device is in the process of paying out a requested value. The value paid at the poll is given in the event data."));

    _addPollEvent(SSP_POLL_DISPENSED,
                  SSP_POLL_INFO,
                  "SSP_POLL_DISPENSED",
                  -1, F("Show the total value the device has dispensed in repsonse to a Dispense command."));
    ...
    _addPollEvent(SSP_POLL_JAMMED,
                  SSP_POLL_ERROR,
                  "SSP_POLL_JAMMED",
                  -1, F("An event showing the hopper unit has jammed and giving the value paid/floated upto that jam.On the smart payout this event is used when a jam occurs during a payout / float / empty operation."));

    _addPollEvent(SSP_POLL_HALTED,
                  SSP_POLL_ERROR,
                  "SSP_POLL_HALTED",
                  -1, F("Triggered when payout is interrupted for some reason."));
...
void NV11::_poll()
{
    ...
    if (dataBus->isDone(&Device))
    {
        ...
        for (uint8_t i = 3; i < Device.Buffer.rxBufferLength; ++i)
        {
