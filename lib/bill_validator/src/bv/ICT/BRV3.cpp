...
int BRV3::_setReclyerCapacity()
{
    uint8_t data[1] = {25};
    uint8_t receiveData[30];
    uint8_t dataCount = 0;
    int status = this->dataBus->SendCommand(0x03, 0x01, 0x23, data, 1, receiveData, &dataCount, true);
    if (status < 0)
    {
        return status;
    }
...
int BRV3::_Poll()
{
    ...
    if ((messageCnt != lastMessageCnt))
    {
        if (messageCnt != 0)
        {
            ...
            if (isInitDone)
                _pollPraser(receiveData[ICTBC_RECEIVE_PACKET_DATA_OFFSET + 1], receiveData[ICTBC_RECEIVE_PACKET_DATA_OFFSET + 2]);
        }
        else
        {
            isInitDone = false;
